...
export const MaskedImage = forwardRef<MaskedImageHandles, ImageMaskProps>(({ src, alt, objectFit }, ref) => {
  ...
  useEffect(() => {
    ...
    context.strokeStyle = theme === 'dark' ? 'rgba(227, 94, 160, 0.7)' : 'rgba(227, 94, 160, 0.7)'; // pink with 0.7 opacity
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', width: '100%', height: '100%' }} className='select-none'>
      <img src={src} alt={alt} style={{ width: '100%', height: '100%', objectFit }} className='select-none'/>
      <canvas ref={canvasRef} style={{ position: 'absolute', top: 0, left: 0, opacity: '50%' }} />
    </div>
  );
})
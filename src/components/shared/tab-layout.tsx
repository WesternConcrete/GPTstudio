...
export default function FileLayout({
    ...
    const [tabValue, setTabValue] = useState(() => {
        const matchedTab = tabs.find(tab => router.asPath.includes(tab.urlFragment));
        ...
        <div className="flex-1 space-y-4 p-8 pt-6">
            <div className="flex items-center justify-between space-y-2">
                <div className="flex gap-2 flex-col">
                    ...
                    <Button className="right-0 top-0 items-center rounded-[0.5rem] text-sm font-medium md:flex px-0" variant="link" size="sm">
            ...
            <Tabs className="space-y-4" value={tabValue}>
                <TabsList>
                    {tabs.map(tab => (
                        <TabsTrigger key={tab.value} onClick={() => router.push(constructUrl(tab.urlFragment))} value={tab.value}>
                            {tab.title}
                        </TabsTrigger>
                    ))}
                </TabsList>
